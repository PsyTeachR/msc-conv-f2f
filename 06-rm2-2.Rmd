...
* Download <a href="files\6.1.zip" download>the chapter 6.1 files </a> and save them in your Chapter 6 folder. Make sure that you do not change the file names at all.  
* Open the Markdown stub file and ensure the working directory is set to your Chapter 6 folder. 
* Typ and run the code that loads the `tidyverse` using the `library()` function and loads the data into an object named `messy` using `read_csv()` in to the Activity 1 code chunk.
...
How many variables does `messy` have? `r mcq(c("4", answer = "5", "6", "7"))`

`r hide("Explain this answer")`
```{r, echo = FALSE, results='asis'}
cat("There are five variables. There are two demographic variables `id` and `gender`. There are two independent variables, `speaker` which is a between-subject variable, and test time which is a within-subject variable. Finally, there is one dependent variable, the participant's score. 
    ")
```
`r unhide()`  
<br>
...
### Activity 6: Questionnaire data

One very common type of data you are likely to work with is questionnaire data.

* Load `questionnaire_data.csv` and `scale_info.csv` into new objects named `qdat` and `scales` using `read_csv()` and then view the objects.  
...
* Join `qdat_tidy` and `scales` by "item" using `inner_join()` and save it to `qdat_tidy` (i.e., overwrite the object). If you need help with this, refer back to Chapter \@ref(join).
...
```{r tidy1, echo = FALSE}
head(qdat_tidy) %>% knitr::kable(digits = 2)
```

### Activity 7: Calculating scale scores
...
head(qdat_scores) %>% knitr::kable(align = "c", digits = 2)
```
...
```{r img-scale, fig.cap = "Boxplots of scale scores"}
ggplot(qdat_scores, aes(scale, mean_score)) +
  geom_boxplot()
```

### Activity 8: `spread()` back to wide-form
...
```{r echo = FALSE}
head(qdat_wide) %>%  knitr::kable(digits = 2, align = "c")
```
...
* Matched pairs design means different people in different conditions but you have matched participants across the conditions (e.g. matched on age, IQ, Social Economic Status, etc.)
...
We will be looking at the dataset from Experiment 4, courtesy of the [Open Stats Lab](https://sites.trinity.edu/osl/data-sets-and-activities/t-test-activities).

>The purpose of this study was to find out whether a person's mental capacities can be inferred from their voice. Professional recruiters either listened to, or read job candidates pitches about why they should be hired, and rated the candidates on intelligence and general impressions. 

The methods and results appear on pages 887--889 of the article if you want more details. The original data, in **wide** format, can be found at the [Open Stats Lab](https://drive.google.com/open?id=0Bz-rhZ21ShvOei1MM24xNndnQ00) website. Today, we will be working with a modified version in "tidy" format which can be downloaded from Moodle. 

### Activity 1: Set-up

Your task is to reproduce the results from the article (p. 887). Do the following. If you need help, consult Chapter \@ref(ref3) and Chapter \@ref(ref2).

* Open R Studio and ensure the environment is clear.   
* Download <a href="files\6.2.zip" download> the chapter 6.2 files</a> and save them in your Chapter 6 folder. Make sure that you do not change the file names at all.
* Open the markdown stub file.
* Type and run the code that loads `broom`, `car`, `lsr`,  and `tidyverse` using the `library()` function and loads the data into an object named `evaluators` using `read_csv()` in the Activity 1 code chunk.
...
There are a few things we should do to explore the dataset and make working with it a bit easier. You have done all of these tasks before, use the book search function if you can't remember how to do them. If you feel confident, do the following data wrangling steps yourself, or run the code below. 

1. Use `mutate()` and `recode()` to recode `sex` into a new variable `sex_labels` so that `1` = `male` and `2` = `female`. Be careful - there are multiple functions in different packages called recode, make sure to specify `dplyr::recode()` to get the right one.
2. Use `mutate()` and `as.factor()` to overwrite `sex_labels` and `condition` as factors.  
3. Use `summary()` to get an overview of the missing data points in each variable.
...
You should **always** visualise your data before you run a statistical analysis. It will help you interpret the results of the test and give you a better understanding of the spread of your data. To visualise our data, we are going to create a violin-boxplot.

`geom_violin()` represents density. The fatter the plot, the more data points there are. If your data are normally distributed it should look something like a violin.  
`geom_boxplot()` shows the median and inter-quartile range (see [here](https://towardsdatascience.com/understanding-boxplots-5e2df7bcbd51) if you would like more information). The boxplot can also give you a good idea if the data are skewed - the median line should be in the middle of the box, if it's not, the data are likely skewed.  
...
summary_dat<-ratings2%>%
  ...
  summarise(mean = mean(Rating),
            min = mean(Rating) - 1.96*sd(Rating)/sqrt(n()), # 95% confidence intervals
            max = mean(Rating) + 1.96*sd(Rating)/sqrt(n()))
...
We can also use the **Shapiro-Wilk** test to statistically evaluate if the data are normal. `shapiro.wilk()` assesses if the distribution is significantly different from a normal distribution. If the test is significant, your data are not normal. If the test is non-significant, your data are approximately normal. 
...
We are going to conduct t-tests for the Intellect, Hire and Impression ratings separately. Each time we are comparing evaluators' overall ratings for the listened group versus overall ratings for the read group to see if the evaluators who **listened** to pitches gave significantly higher or lower ratings than evaluators that **read** pitches.
...
Most of the work of statistics involves the set-up - running the actual tests is very simple. To conduct the t-test we will use the function `t.test()`. This function uses a style of code you haven't come across yet but that is very important to get used to, **formula syntax**.
...
Because we've run three t-tests we risk inflating our chances of a Type 1 errors due to familywise error and we should apply a correction for multiple comparisons.

To do this, we need to join all the results of the t-tests together using `bind_rows()`. First, you specify all of the individual tibbles you want to join and give them a label, and then you specify what the ID column should be named.
...
Our next task is to calculate the effect size (for a t-test it's Cohen's D). We will use the function `cohensD()` from the `lsr` package. The code is very similar to the syntax for `t.test()`. The only difference is rather than `paired = FALSE`, you must specify `method = "unequal"` which indicates that we conducted a Welch test (see the help documentation for more information). 
...
For the final activity we will run a paired-samples t-test for a within-subject design. For this example we will again draw from the [Open Stats Lab](https://sites.trinity.edu/osl/data-sets-and-activities/t-test-activities) and look at data from the following paper:
...
This study examines the hypothesis that particular melodies sung by the parents convey information about social affiliation to infants. The songs children are exposed to growing up depend on the culture or social group. Thus, when a stranger sings a familiar song, it may signal to the infant that this new person is a member of their social group.

The researchers recruited 32 infants and their parents. The parents were taught a new lullaby and asked to sing it to their child every day for the next 1-2 weeks. The parents and their infant then returned to the lab. As a 'baseline', the infants were first shown videos of two unfamiliar people, each silently smiling and looking at the infant, and the infants' gaze was measured. Next, the unfamiliar people sang either the lullaby that the parents learned or a lullaby with a different melody. Finally, the infants saw the same silent video used at baseline, and the researchers again recorded the gaze of the infants during this ‘test’ phase. 


* First, run the below code to wrangle `Mehr Song and Spelke 2016 Experiment 1.csv` into the format we need - this code selects only the data we need for the analysis and renames variables to make them easier to work with.
...
The key difference is that for the paired-samples test, the assumption of normality is that the differences between each pair of scores are normally distributed, rather than the scores themselves.
...
* Run the below code to tidy the data and calculate descriptives and then create the same violin-boxplot as you did for the independent t-test (you can copy and paste the code from above and change the data and variable names).
